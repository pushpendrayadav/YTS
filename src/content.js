...
if (window.__ytSummarizerContentLoaded) {
  console.log(" content.js already loaded on page:", window.location.href);
} else {
  window.__ytSummarizerContentLoaded = true;
    ...
    function extractTranscriptFromDOM() {
      ...
      for (const selector of selectors) {
        try {
          ...
          console.log(
            ` Selector "${selector}" found ${panels.length} elements`,
          );
      ...
      if (segments.length === 0) {
        ...
        const allText =
          transcriptPanel.innerText || transcriptPanel.textContent;
